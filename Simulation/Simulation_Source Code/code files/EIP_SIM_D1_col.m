% nachfolgende Funktion ist Produktionssystem Heuristik D1
...
% Ein kompletter Durchgang iterated elimination of dominated strategies betrachtet die vollständigen Strategien beider Spieler
...
% If CHANGE_count = 1 ,	Current max = empty. MOVE to first (nearest) strategy, first cell. First strategy = current strategy. First cell = current cell. Next MOVE = MOVE 1.
% If CHANGE_count = 2 , (If focus changed twice,)	CHOOSE a strategy from remaining strategy set randomly.
...
while (EIP_diff>epsilon || choice_diff>epsilon || remain_strat_diff>epsilon || sim_count<sim_min)
    ...
    remain_alter_opp=ones(1,anz_strat);
    comp_strat_1=0;
    comp_strat_2=0;
    count_cell_comp=0;
    max_strat_count=0;
    ...
    %one rounde eliminating dominated strategies = go through both players' strategies
    for i=1:2
        %neu Schleife
        if time_break==1
            break;
            ...
            for gg=g+1:anz_strat
                ...
                for m=1:anz_strat
                    if it_dom_matrix(rnd_row(m))==1
                        ...
                        %neu ggf. in switch integrieren?!
                        if count_EIPs<EIP_max_time-1
                            count_STORE=count_STORE+1;
                            count_EIPs=count_EIPs+1;
                        end
                        % neu schon bei switch erfasst
                        %if count_EIPs<EIP_max_time-1
                        %count_COMPARE_I=count_COMPARE_I+1;
                        % count_EIPs=count_EIPs+1;
                        %end
                        ...
                        if neutr~=1
                            if max_strat_alt ~= 0
                                if max_strat~=max_strat_alt
                                    ...
                                    max_strat_count=0;
                                    neutr_count=0;
                                    %neu eliminate hier falsch, festgestellt wurde, dass sich beide Strategien nicht dominieren.
                                    %if count_EIPs<EIP_max_time-1
        ...
        if focus
            focus=0;
            remain_alter_own=dom_matrix; %Ä col
        else
            focus=1;
            remain_alter_opp=dom_matrix; %Ä col
        end
        it_dom_matrix=dom_matrix;
        max_strat=0;
        max_strat_alt=0;
        max_strat_count=0;
        neutr_count=0;
                ...
                for g=1:anz_strat %col
                    ...
                    if remain_alter_opp(g)==1
                        ...
                        if count_EIPs<EIP_max_time-1
                            ...
                            count_EIPs=count_EIPs+1;
                            current_sum=current_sum+game_matrix(g,2*rnd_row_n(h)); %Ä col
                        else
                            break_ok=1;
                            time_break=1;
                            break;
