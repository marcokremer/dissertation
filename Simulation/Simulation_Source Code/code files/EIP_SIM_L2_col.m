...
while (EIP_diff>epsilon || choice_diff>epsilon || remain_strat_diff>epsilon || sim_count<sim_min)
    ...
    for g=1:anz_strat %row
        for h=1:anz_strat %col
            ...
            current_sum=current_sum+game_matrix(g,2*rnd_row(h)-1);
            if count_EIPs<EIP_max_time-1
    ...
    for h=1:anz_strat
        ...
        if game_matrix(strat_op_choice,2*rnd_row(h))>current_sum
        current_sum=game_matrix(strat_op_choice,2*rnd_row(h));
