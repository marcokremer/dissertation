% nachfolgende Funktion ist Produktionssystem Heuristik D2
...
% If CHANGE_count < 4,	Current max = empty. MOVE to first (nearest) strategy, first cell. First strategy = current strategy. First cell = current cell. Next MOVE = MOVE 1.
% (If CHANGE_count = 4 ,	CHOOSE a strategy from remaining strategy set
% randomly.) old!
% If CHANGE_count = 4 ,	CHOOSE the Naiive strategy from remaining strategy set
...
max_strat=0;
max_change=0;
comp_matrix=zeros(size(game_matrix));
...
for i=1:4
    ...
    switch i
        ...
        otherwise
            warning('Unexpected number for i.');
    end
