function plot_effectivity_goal3_LS(X1,fitting_vorschrift_array,fitting_vorschrift_array_sat, xu_lim,yu_lim,yl_lim, sat_point, g)
...
%  X1(ceil((xu_lim-5)/3)+i)=X1(ceil((xu_lim-5)/3))+3;
 YMatrix1(i)=fitting_vorschrift_array_sat{i}(X1(end));
end
f=g;
saturierung=[15 94 224 439]; %für Levels of Complexities
...
figure_name=strcat('Effectivity_Goal_3_LS_Size_', num2str(f+1),'x',num2str(f+1),'_Phase');

fig_title=strcat({'Effectivity: Goal 3 -'},{' '},num2str(f+1),{'x'},num2str(f+1),{' '},{'- LS'});
...
set(plot1,'DisplayName','Altruist', 'Color',[0 0 0]+0.8);%shade of grey
set(plot2,'DisplayName','Random', 'Color',[0 0 0]+0.7);%shade of grey
set(plot3,'DisplayName','Optimist', 'Color',[0 0 0]+0.6);%shade of grey
set(plot4,'DisplayName','Pessimist', 'Color',[0 0 0]+0.5);%shade of grey
set(plot5,'DisplayName','Naiive', 'Color',[0 0 0]+0.4);%shade of grey
set(plot6,'DisplayName','L2', 'Color',[0 0 0]+0.3);%shade of grey
set(plot7,'DisplayName','D1', 'Color',[0 0 0]+0.2);%shade of grey
set(plot8,'DisplayName','D2', 'Color',[0 0 0]+0.1);%shade of grey
set(plot9,'DisplayName','Equilibrium', 'Color',[0,0,0]);%black

pos_vec={'N','N','NE','SE','S','NW','NW','SW','SW','N'};
lab_vec={'Al','Ra','Op','Pe','Na','L2','D1','D2','Eq','pEq'};
buf_vec=[0 0 0.1 0.1 0 0 0 0 0];
[~,~,ic] = unique(YMatrix1);

for ggg=1:max(ic)
    lab=find(ic==ggg);
if size(ic(ic==ggg),1)>1
    
    lab_val={lab_vec{lab(1)}};
    for gg=2:size(ic(ic==ggg),1)
    lab_val=strcat(lab_val,'/',lab_vec{lab(gg)});
    end
labelpoints(xu_lim(g), YMatrix1(lab(1)), lab_val{1},'SW');
else
    labelpoints(xu_lim(g), YMatrix1(lab(1)), lab_vec{lab(1)},'SW');%
end
end
