...
/* Funktionen
function multi2dSortAsc(&$arr, $key1, $key2) in nirgendwo -->raus
function extractTag($datastr, $tagstr) 1x in unfoldEvent
function unfoldEvent($rowarr) 6x in Anweisung ohne Funktion
function unpackProc($procdata) 1x in Anweisung ohne Funktion
function ExpfromDB($nameofexp,$table)  3x in Anweisung ohne Funktion
function objEvent(event,name,value,time) 1x Anweisung ohne Funktion
function showData(formlink) 1x in playData --> raus
function playData() 1x in Anweisung ohne Funktion (2er Teil am Ende) --> raus
function replay(exp1) 1x in Anweisungen ohne Funktion (2er Teil am Ende) --> raus
function download() 1x in Anweisungen ohne Funktion (2er Teil am Ende)
function process() 1x in Anweisungen ohne Funktion (2er Teil am Ende)
function selecting(selType) 1x in Anweisungen ohne Funktion (2er Teil am Ende)
*/
...
$sqlQuery = "SELECT * from $table where expversion='".$nameofexp."'";
    ...
    { 
        ...
        if(!($i%2)) 
			...
			# note that this routine assumes the addvar list is the same for all rows with this expversion
...
{ // start action part (caused by a submitted form)
	...
	if ($act=="download") 
		...
		foreach ($_POST as $key => $value) { 
		 if (substr($key,0,1)=="v") {$arr[$i]=$value;$i++;}
    		}
...
for ($expcount=0;$expcount<count($arr);$expcount++)
{
//hier wird der Pfad angegeben, wo die Auswertedatei als Datenbankauszug abgelegt zu finden ist.
$filename = $arr[$expcount].".csv";
...
$i=0;
		...
		// generate array with numerical indexes for all the fields that have to be added
			...
			// trick: walk through file in backward order to know the maxcount of each trial
			...
			$filename = $arr[$expcount]."_proc.csv";
			...
			$filename = $arr[$expcount]."_proc_sum".$division.".csv";
...
for ($expcount=0;$expcount<count($arr);$expcount++)
{
//hier werden die Dateinamen für die Auswertedatei festgelegt
$filename1 = $arr[$expcount]."_proc.csv";
$filename2 = $arr[$expcount]."_proc_sum".$division.".csv";
...
<?php
	...
	if ($act=="show") 
		{
		//start if show part
		$expversion = $_POST['expversion'];
		$outstr="";

		$rowarr = ExpfromDB($expversion,$table);
			//if ($_POST['unpack']==="true") $outarr = unfoldEvent($rowarr); else $outarr = $rowarr;
			for ($j=0;$j<count($rowarr);$j++)
				{ 
				if ($_POST['unpack']==="true") 
...
<body>
<H1>Results of Experiment <?php echo($expversion);?></H1>
<Table border=1>
<?php echo($outstr); ?> 
</TABLE>
...
<?php
	...
	$sqlQuery = "SELECT id, subject, submitted, procdata, condnum from $table where expversion='$expversion'";
...
if (document.forms[0].subj.value!="")
	...
	playexp = "<?php echo($expversion);?>.php";
...
<body>
<H1>Playback for experiment: <?php echo($expversion);?></H1>
<form>
...
<p><strong>Note:</strong>The Replay routine assumes that the php-file you want to replay has the same name as the experiment itself: <?php echo($expversion);?>.php . You can change the name of the file on the replay page if it is different from the file name.</p>
</form>
</body>
</HTML>
<?php
...
$sqlQuery = "SELECT DISTINCT expversion from $table";
...
document.forms[0].expversion.value=exp1;
...
<form method="post" action="mdatalyser.php">
...
<TR><TH>Experimentversion</TH><TH>Download</TH><TH>Show data</TH><TH>Play back</TH></TR>
<?php
	// Make rows for records 
    while($rec=mysql_fetch_row($result)) 
    { 
        echo "<TR><TD>$rec[0]</TD><TD align=center><input type=checkbox name=\"$rec[0]\" value=\"$rec[0]\" CHECKED><TD><input type=button value=\"Show Table\" onClick=\"show('$rec[0]')\"></TD><TD>";
		// check if there is any process data before creating a play back button
		$playstr="&nbsp;";
		$sqlQuery = "SELECT expversion from $table where expversion='$rec[0]'";
	    # Execute SQL query 
...
for (i=0;i<document.forms[0].elements.length;i++)
	{
	if ((document.forms[0].elements[i].type=='checkbox') & (document.forms[0].elements[i].name.substr(0,3)=="exp"))
		{
