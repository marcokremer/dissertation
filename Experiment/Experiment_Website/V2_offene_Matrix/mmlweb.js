...
// 140504 20:43 Uhr Umfangreiche Änderungen analog mmlweb.js Version 1 durchgeführt: erwartungswert_zwei ohne Variablenübergabe und Aufruf über setTimeout in Fkt StartTmBar(); 
//Einfügen der Fkten  function richtige_wahl(), function Wechsel(), die für die Anzeige des Nash-Ggw als grüner Blinker erstellt wurden;
//Einfügen der Fkten function beenden(), die erst submitet nachdem die Fkt richtige_Wahl() ausgeführt wurde.
//140706 22:00 Uhr Cookies_Losnummer wird erst in letzter Runde eingetragen (siehe Fkt'en Erwartungswet/_2)
//140812 06:50 Uhr Aufnahme der Mausbewegung in Speicherstring (x-Koordinate in einer Matrixzelle) zur erweiterten Analyse der Mausbewegung.
...
function erwartungswert_zwei() //wird in mmlweb.js in Fkt refreshTmBar() aufgerufen; übergibt weniger Variablen als Fkt erwartungswert
*/
...
loaded = false; // flag to test whether page has been loaded is set to true in reorder
...
//3. Wird er ein zweites Mal angeklickt, erählt er den btnstyle
// Konstanten:
...
//Funktion nachfolgender Zeile ist mir nicht bekannt, wird auch an keiner Stelle weiter verwand. 
//Vlt. bei Datenspeicherung?
//if (document.forms[mlweb_fname].choice){document.forms[mlweb_fname].choice.value = name;}

	if (btnType=="button")
	{
	//Der eval()-Aufruf muss an dieser Stelle geschehen, damit die HTML-Seite 
	//vor dem Aufruf der nächsten *.js-Zeilen einmal geladen ist und die <input>
	//-Container definiert und befüllt sind. Anschließend greift die Funktion
	//recCHoice auf die Container-Inhalte zu,prüft den Inhalt und verändert sie.
	HandleBut = eval("document.forms['"+mlweb_fname+"']."+name);
	
	if (HandleBut.className == 'btnStyle btnhov')
		{
		HandleBut.className = 'pressedStyle btnhov';
		}else
		{HandleBut.className = 'btnStyle btnhov';
		}
	}	
}
