...
for a=1:3 %Experimentversionen
    ...
    no_abs_durchschnittl_hypo_erg=cell2mat(cluster_cen.(temp_cluster_list{c}));
    no_abs_durchschnittl_hypo_erg(isnan(no_abs_durchschnittl_hypo_erg))=0;
...
%     fig_name=strcat('Typen_',temp_cluster_list{c});
    ...
    switch logical(c)
        case ~isempty(strfind(temp_cluster_list{c},cluster_auswahl_name{1}))
            ...
            new_order_hypo_erg_abs=no_abs_durchschnittl_hypo_erg(:,[12,15,11,14,3,6:10,1:2,4:5,13]);
...
%             anz_typ=3;
        ...
        case ~isempty(strfind(temp_cluster_list{c},cluster_auswahl_name{2}))
            ...
            new_order_hypo_erg_abs=no_abs_durchschnittl_hypo_erg;
            xlim=3;
            l=0;
            d=2;
%             anz_typ=2;
...
%     idx_data_zero=find(sum(new_order_hypo_erg,2)==0);
    ...
    if max(sum(typen_matrix))>1
        ...
        new_order_hypo_erg_n_abs=new_order_hypo_erg_abs;
...
%             for m=1:size(idx_max_clustersize,2)
            ...
            new_order_hypo_erg_n_abs(idx_cluster,:)=[];
...
%     idx_data_zero=find(sum(new_order_hypo_erg_n,2)==0);
    ...
    new_order_hypo_erg_n(idx_data_zero,:)=[];
     new_order_hypo_erg_n_abs=typen_matrix'*new_order_hypo_erg_n_abs;
    new_order_hypo_erg_n_abs(idx_data_zero,:)=[];
...
%     idx_data_zero=find(sum(new_order_hypo_erg_n,2)==0);
    ...
    new_order_hypo_erg_n(idx_data_zero,:)=[];
    new_order_hypo_erg_n_abs=typen_matrix'*new_order_hypo_erg_abs;
    new_order_hypo_erg_n_abs(idx_data_zero,:)=[];
...
%     temp_new_order_hypo_erg_n=new_order_hypo_erg_n(sum(new_order_hypo_erg_n,2)>0,:);
        ...
        temp_new_order_hypo_erg_n_abs=new_order_hypo_erg_n_abs;
    for g=1:size(idx_types,1)
        ...
        if ~isfield(type_centers,type_centers_name)
            ...
            type_centers_abs.(type_centers_name)= temp_new_order_hypo_erg_n_abs(g,:);
        else
            type_centers.(type_centers_name)= vertcat(type_centers.(type_centers_name), temp_new_order_hypo_erg_n(g,:));
        type_centers_abs.(type_centers_name)= vertcat(type_centers_abs.(type_centers_name), temp_new_order_hypo_erg_n_abs(g,:));
        end
    end
% if sum(k(:,1))>0
...
save('type_centers.mat','type_centers', 'type_centers_abs');